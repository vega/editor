...
class ShareModal extends React.PureComponent<Props, State> {
  ...
  public updateGist() {
    ...
    if (this.state.gistId) {
      const cookieValue = encodeURIComponent(getCookie(COOKIE_NAME));
      fetch(`${BACKEND_URL}gists/update`, {
        body: JSON.stringify({
        ...
        if (res.status === 205) {
          this.setState({
            gistFileName: this.state.gistFileNameEdited,
            updated: true,
          });
        }
      });
        ...
        <div className="share-split">
          <div>
            ...
            <p>
              ...
              algorithm. When whitespaces are not preserved, the editor will automatically format the specification when
              it is loaded.
            </p>
            <div>
          ...
          <div>
            ...
            <div className="share-input-container">
              <button onClick={this.updateGist.bind(this)}>{this.state.updated ? 'Update' : 'Updating...'}</button>
            </div>
          </div>
        </div>
      </>
